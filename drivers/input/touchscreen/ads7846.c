...
struct class *anusha;
static struct kobject *my_device1_kobj;
...
SHOW(anusha_input, vaux, vaux_adjust)
...
static ssize_t sys_x_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    /* Read the value of sys_x from the driver or related data structure */
    //unsigned sys_x = 20; // Example value, replace with actual value

    return sprintf(buf, "%u\n", sys_x);
}

static ssize_t sys_y_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    /*Read the value of sys_y from the driver or related data structure */
   // unsigned sys_y = 100; // Example value, replace with actual value
...
static void ads7846_report_state(struct ads7846 *ts)
{
	 ...
	 * value can fluctuate for quite a while after lfting the pen and
...
static int ads7846_probe(struct spi_device *spi)
{
	...
	int err;
	/*struct kobject *anusha_kobj=kobject_create_and_add("anusha",&class_kset->kobj);
        if(!anusha_kobj){
           printk(KERN_ERR "Failed to create kobject for anusha\n");
           return -ENOMEM;
        }*/
	//CREATE SYSFS ATTRIBUTE GROUP FOR ANUSHA
	/*if (sysfs_create_group(anusha_kobj, &ads784x_attr_group) !=0){
	    printk(KERN_ERR "Failed to create attribute group for anusha\n");
	...
	if(!my_device1_kobj) {
        ...
        // Create the sysfs attribute group for my_device1
        if (sysfs_create_group(my_device1_kobj, &ads784x_attr_group) != 0) {
            printk(KERN_ERR "Failed to create attribute group for my_device1\n");
            kobject_put(my_device1_kobj);
            return -ENOMEM;
        }
	...
	if (err) {
		...
		printk(KERN_INFO "value is _______________&&&&&&&&&&&&&&*******************8________%p\n",&spi->dev);
	...
	if (err && !pdata->irq_flags) {
		...
		printk(KERN_INFO "value _____-***************^^^^^^^^^%p\n",&spi->dev);
...
// Additional initialization code for your device
...
{
    // Cleanup and release any resources here

    // Destroy the kobject for your device
    kobject_put(my_device_kobj);
}*/
